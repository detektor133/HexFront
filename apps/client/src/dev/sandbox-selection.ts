// Выбор и приказы кликом в песочнице (03/T12): тап по гексу со своими отрядами выбирает их,
// следующий тап — «идти» на свободный гекс или прицел атаки на вражеский (с прогнозом).
...
/**
 * Тап по гексу.
 * @returns новый выбор и команда, если тап — приказ «идти»
 */
export function tapHex(
  view: PlayerView,
  picked: Picked,
  hex: number,
): { readonly picked: Picked; readonly cmd: Command | null } {
  const mine = pickedUnits(view, picked);
  const elsewhere = mine.length > 0 && mine.some((u) => u.hex !== hex);
  if (elsewhere) {
  ...
  // Повторный тап по гексу выбранных отрядов снимает выбор отрядов.
  const again = mine.length > 0;
  const units = again ? [] : ownUnitsAt(view, hex).map((u) => u.id);
  return { picked: { hex, units, target: null }, cmd: null };
}
