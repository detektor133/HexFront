...
- `stage-02/economy-1440x900.png` — вся карта, выбрана столица: карточка города, кнопки с ценой и причиной отказа.
- `stage-02/economy-1440x900-zoom.png` — масштаб 1,6, выбран свой гекс.
- `stage-02/economy-390x844.png` — телефон: отладочная панель занимает большую часть экрана (временный вид; финальный HUD — этап 04).

## `pnpm check`

```
typecheck — ok; eslint + prettier — ok
✔ no dependency violations found (116 modules, 337 dependencies cruised)
tokens: up to date
...
В `QUESTIONS.md` — допущения T10 на подтверждение: состав локального матча (2 игрока, соперник без действий), отладочное поле роста в `playerView`, тонкие дороги на `/dev/economy`. В `CHANGELOG-proposals.md` с этапа 01 ждут решения стиль дорог (4 конфликта) и синие цвета игроков против воды.
